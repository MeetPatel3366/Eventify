...
const EventProgress = () => {
  ...
  const { progressEvent, loading } = useSelector((state) => state.event);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
          {progressEvent.map((event) => {
            ...
            return (
              <div
                key={event._id}
                className="group relative bg-white/5 backdrop-blur-md rounded-3xl border border-white/10 overflow-hidden hover:border-indigo-500/50 hover:scale-[1.02] transition-all duration-300 flex flex-col shadow-2xl"
              >
                ...
                <div className="p-5 flex-grow flex flex-col">
                  <h2 className="text-lg font-bold mb-3 line-clamp-1 group-hover:text-indigo-400 transition-colors">
                    {event.name}
                  </h2>
                  ...
                  <div className="mt-auto space-y-3 bg-white/5 p-4 rounded-2xl border border-white/5">
                    ...
                    <div className="relative w-full h-2 bg-gray-800 rounded-full overflow-hidden shadow-inner">
                      ...
                      <div
                        className={`h-full rounded-full ${config.bg} transition-all duration-1000 ease-in-out`}
